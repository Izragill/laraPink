        ...
        @foreach($articles as $article)
            <div class="sticky hentry hentry-post blog-big group">
                ...
                <div class="meta group">
                    ...
                    <p class="categories">
                        <span>In: <a href="{{ route('articlesCat', ['cat_alias' => $article->category->alias]) }}"
                                     title="View all posts {{ $article->category->title }}"
                                     rel="category tag">{{ $article->category->title }}</a>
                        </span>
                    </p>
                    <p class="comments"><span>
                            <a href="{{ route('articlesCat', ['cat_alias' => $article->category->alias]) }}#respond" title="Comment on Section shortcodes &amp; sticky posts!">
                                {{ count($article->comment) > 0 ? count($article->comment) : '0' }} {{ Lang::choice('ru.comments', count($article->comment)) }}
                            </a>
                    </span></p>
                </div>
                <!-- post content -->
