<div id="content-single" class="content group">
    ...
    <div id="comments">
        <h3 id="comments-title">
            <span>{{ count($article->comment) ? count($article->comment) : '0' }}</span> {{ mb_strtolower(Lang::choice('ru.comments', count($article->comment))) }}
        </h3>
        ...
        <div id="respond">
            <h3 id="reply-title">{!! Lang::get('ru.comment_leave_reply') !!} <small><a rel="nofollow" id="cancel-comment-reply-link" href="#respond" style="display:none;">{{ Lang::get('ru.comment_cancel_reply') }}</a></small></h3>
            <form action="{{ route('comment.store') }}" method="post" id="commentform">
                @csrf

                @if(!Auth::check())
                    <p class="comment-form-author"><label for="author">{{ Lang::get('ru.comment_name') }}</label> <input id="author" name="name" type="text" value="" size="30" aria-required="true" /></p>
                    <p class="comment-form-email"><label for="email">{{ Lang::get('ru.comment_email') }}</label> <input id="email" name="email" type="text" value="" size="30" aria-required="true" /></p>
                    <p class="comment-form-url"><label for="url">{{ Lang::get('ru.comment_website') }}</label><input id="url" name="site" type="text" value="" size="30" /></p>
                @endif

                <p class="comment-form-comment"><label for="comment">{{ Lang::get('ru.comment_your_comment') }}</label><textarea id="comment" name="text" cols="45" rows="8"></textarea></p>
                <div class="clear"></div>
                <p class="form-submit">
                    <input id="comment_post_ID" type="hidden" name="comment_post_ID" value="{{ $article->id }}" />
                    <input id="comment_parent" type="hidden" name="comment_parent" value="0" />

                    <input name="submit" type="submit" id="submit" value="{{ Lang::get('ru.comment_post_comment') }}" />
                </p>
            </form>
        </div>
        <!-- #respond -->
